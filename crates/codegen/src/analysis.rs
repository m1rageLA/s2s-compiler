...
fn usage_in_stmts(name: &str, stmts: &[IrStmt]) -> Usage {
    stmts
        .iter()
        .fold(Usage::default(), |acc, stmt| acc.combine(usage_in_stmt(name, stmt)))
}
...
fn usage_in_stmt(name: &str, stmt: &IrStmt) -> Usage {
    match stmt {
        ...
        IrStmt::DoWhile(body, condition) => usage_in_stmts(name, body).combine(usage_in_expr(name, condition)),
        IrStmt::For {
            init,
            condition,
            update,
        ...
        IrStmt::Switch {
            discriminant,
            cases,
        } => cases.iter().fold(usage_in_expr(name, discriminant), |acc, case| {
...
fn usage_in_expr(name: &str, expr: &IrExpression) -> Usage {
    match expr {
        ...
        IrExpression::Binary { left, right, .. } => usage_in_expr(name, left)
            .combine(usage_in_expr(name, right)),
        ...
        IrExpression::Template(parts) => parts.iter().fold(Usage::default(), |acc, part| {
            match part {
        ...
        IrExpression::Object(properties) => properties.iter().fold(Usage::default(), |acc, prop| {
...
fn usage_in_runtime_call(name: &str, call: &RuntimeNamespace) -> Usage {
    match call {
        RuntimeNamespace::Array(array_call) => match array_call {
            ...
            | ArrayCall::Filter { target, callback } => usage_in_expr_target(name, target)
                .combine(usage_in_expr(name, callback)),
            ArrayCall::Join { target, separator } => usage_in_expr_target(name, target)
                .combine(usage_in_expr_opt(name, separator.as_deref())),
        },
        RuntimeNamespace::Console(ConsoleCall::Log(args)) => args.iter().fold(
            Usage::default(),
            |acc, expr| acc.combine(usage_in_expr(name, expr)),
        ),
...
fn usage_in_string_call(name: &str, call: &StringCall) -> Usage {
    match call {
        ...
        StringCall::Concat { target, args } => args
            .iter()
            .fold(usage_in_expr(name, target), |acc, expr| {
                acc.combine(usage_in_expr(name, expr))
            }),
...
fn usage_in_value_call(name: &str, call: &ValueCall) -> Usage {
    match call {
        ...
        | ValueCall::GreaterThanOrEqual { left, right } => usage_in_expr(name, left)
            .combine(usage_in_expr(name, right)),
        ValueCall::LogicalNot { expr } => usage_in_expr(name, expr),
        ValueCall::GetProperty { target, .. } => usage_in_expr(name, target),
        ValueCall::GetPropertyDynamic { target, property } => usage_in_expr(name, target)
            .combine(usage_in_expr(name, property)),
    }
}
