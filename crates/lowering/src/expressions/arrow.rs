...
mod tests {
    ...
    fn lowers_arrow_expression_bodies() {
        ...
        let (params, body) = match double.value.as_ref().expect("double should have initializer") {
        ...
        match body {
            IrArrowBody::Expr(expr) => match expr.as_ref() {
                IrExpression::Binary { op: IrBinOp::Mul, .. } => {}
        ...
        let (params, body) = match format.value.as_ref().expect("format should have initializer") {
            IrExpression::Arrow { params, body } => (params.clone(), body.clone()),
