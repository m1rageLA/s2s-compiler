    ...
    lookup_type_alias(id)
        .and_then(|alias| match alias.def {
            IrTypeAliasDef::Object(fields) => {
                Some(fields.iter().all(|field| is_copy_type(&field.ty)))
            }
            _ => None,
        })
        .unwrap_or(false)
...
fn infer_runtime(call: &RuntimeNamespace) -> Option<IrType> {
    match call {
        ...
        RuntimeNamespace::Array(ArrayCall::Push { target, .. }) => {
            match infer_expression_type(target) {
                Some(IrType::Array(
                    IrArrayKind::Number
                    | IrArrayKind::Str
                    | IrArrayKind::Bool
                    | IrArrayKind::Object(_),
                )) => Some(IrType::UInt),
                _ => Some(IrType::Value),
            }
        }
        RuntimeNamespace::Array(ArrayCall::Length { target }) => {
            match infer_expression_type(target) {
                Some(IrType::Array(
                    IrArrayKind::Number
                    | IrArrayKind::Str
        ...
        RuntimeNamespace::Array(ArrayCall::Index {
            element, target, ..
        }) => match element {
...
pub fn is_copy_type(ty: &IrType) -> bool {
    matches!(
        ty,
        IrType::Number | IrType::UInt | IrType::Bool | IrType::Unit
    )
}
