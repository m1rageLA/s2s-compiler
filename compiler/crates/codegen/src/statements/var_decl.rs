use ir::{
    ArrayCall, IrArrayKind, IrArrowBody, IrExpression, IrParam, IrType, IrVariable,
    RuntimeNamespace,
};
use proc_macro2::TokenStream;
use quote::quote;

use crate::{Codegen, analysis, typing};
...
pub fn var_decl_tokens(vars: &[IrVariable]) -> TokenStream {
    for var in vars {
        ...
        if let (
            IrType::Object(_),
            Some(IrExpression::RuntimeCall(RuntimeNamespace::Array(ArrayCall::Index {
                target,
                index,
                element,
            }))),
...
fn function_param_passes_from_initializer(
    expr: Option<&IrExpression>,
) -> Option<Vec<typing::ParamPass>> {
    match expr {
