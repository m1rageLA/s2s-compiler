use ir::{
    ArrayCall, IrArrayKind, IrAssignOp, IrExpression, IrType, IrTypeAliasDef, RuntimeNamespace,
};
use proc_macro2::{Literal, Span, TokenStream};
use quote::{format_ident, quote};

use crate::{Codegen, expression::object_struct_literal_tokens, typing};
...
pub(crate) fn assignment_tokens(
    ...
    match op {
        ...
        IrAssignOp::ExpAssign => exponent_assign_tokens(
            &target_ident,
            &rhs_ident,
            &left_tokens,
            &right_tokens,
...
fn array_index_target(
    left: &IrExpression,
) -> Option<(&IrExpression, &IrExpression, Option<IrArrayKind>)> {
    match left {
...
fn array_index_assignment_tokens(
    ...
    let number_op = |op_tokens: TokenStream| {
        let coerced_rhs =
            typing::coerce_to_type(quote! { (#right_tokens) }, &IrType::Number, right_ty);
        quote!({
    ...
    match op {
        IrAssignOp::Assign => {
            let coerced_rhs =
                typing::coerce_to_type(quote! { (#right_tokens) }, &element_ty, right_ty);
            let assign_value = if typing::is_copy_type(&element_ty) {
                quote! { *#target_ident = #value_ident; }
            } else {
                quote! { *#target_ident = (#value_ident).clone(); }
        ...
        IrAssignOp::AddAssign => {
            if matches!(element_ty, IrType::Number) {
                ...
                let rhs =
                    typing::coerce_to_type(quote! { (#right_tokens) }, &IrType::Str, right_ty);
                quote!({
                    let #idx_ident = #idx_tokens;
                    let #rhs_ident = #rhs;
                    let #vec_ident = #vec_tokens;
        ...
        | IrAssignOp::NullishCoalesceAssign => {
            unsupported_assign_op("unsupported array assignment")
        }
    }
}
...
fn member_assignment_tokens(
    ...
    match op {
        ...
        IrAssignOp::AddAssign => {
            member_value_op(quote!(runtime::value::ops::add), object, property, right)
        }
        IrAssignOp::SubAssign => {
            member_value_op(quote!(runtime::value::ops::sub), object, property, right)
        }
        IrAssignOp::MulAssign => {
            member_value_op(quote!(runtime::value::ops::mul), object, property, right)
        }
        IrAssignOp::DivAssign => {
            member_value_op(quote!(runtime::value::ops::div), object, property, right)
        }
        IrAssignOp::ModAssign => {
            member_value_op(quote!(runtime::value::ops::modulo), object, property, right)
        }
        ...
        | IrAssignOp::LogicalAndAssign
        | IrAssignOp::NullishCoalesceAssign => {
            unsupported_assign_op("unsupported member assignment")
        }
    }
}
