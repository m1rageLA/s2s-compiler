use ir::{
    ...
    IrPrefixOp, IrType, IrUnaryOp, RuntimeNamespace,
};
...
fn array_index_postfix_tokens(
    ...
    let element_ty =
        match typing::infer_expression_type(target).or_else(|| element.map(IrType::Array)) {
...
pub(crate) fn unary_tokens(op: &IrUnaryOp, expr: &IrExpression) -> TokenStream {
    match op {
        IrUnaryOp::TypeOf => match typing::infer_expression_type(expr) {
