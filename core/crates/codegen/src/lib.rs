            ...
            match item {
                IrItem::Variable(var) => {
                    ...
                    if let Some((params, ret)) =
                        function_signature_from_initializer(var.value.as_ref())
                    {
                        typing::define_function_params(&var.name, &params);
                        typing::define_function_return(&var.name, ret);
                        if let Some(passes) =
                            function_param_passes_from_initializer(var.value.as_ref())
                        {
                ...
                syn::Expr::Closure(closure) => {
                    ...
                    match &closure.inputs[0] {
                        syn::Pat::Type(pat_type) => match pat_type.ty.as_ref() {
                            syn::Type::Path(path) => {
                                assert_eq!(quote!(#path).to_string(), quote!(f64).to_string());
                            }
