    ...
    match (object, property) {
        ...
        (IrExpression::Identifier(_), "push") => {
            ...
            let args = if matches!(
                target_ty,
                Some(IrType::Array(
                    IrArrayKind::Any | IrArrayKind::Value | IrArrayKind::Unknown
                )) | Some(IrType::Any)
                    | Some(IrType::Value)
                    | None
            ) {
...
mod tests {
    ...
    use ir::{IrExpression, IrItem, IrLiteral, MathCall, RuntimeNamespace, StringCall, ValueCall};
